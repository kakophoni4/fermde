...
let phones=[],loading=false,requestGeneration=0,openGeneration=0;
dialog.addEventListener('close',()=>{requestGeneration++;openGeneration++});
document.addEventListener('fermde:logout',()=>{requestGeneration++;openGeneration++;phones=[];$('#file-list').replaceChildren();$('#file-source').replaceChildren();$('#file-target').replaceChildren()});
...
async function api(path,method='GET',body){
 ...
 if(r.status===401)document.dispatchEvent(new Event('fermde:unauthorized'));
...
async function refresh(){
 const generation=++requestGeneration;const source=$('#file-source').value;$('#file-list').replaceChildren();
 $('#file-destination').classList.toggle('hidden',source!=='server');
 const data=await api(source==='server'?'/files':`/devices/${source}/files`);
 if(generation!==requestGeneration||!dialog.open||source!==$('#file-source').value)return;
 if(source==='server'){
...
async function open(){
 if(dialog.open)return;dialog.showModal();const generation=++openGeneration;$('#file-list').replaceChildren();message('Загрузка…');
 try{
  const result=await api('/devices');if(generation!==openGeneration||!dialog.open)return;phones=result.filter(p=>p.status==='running');
  ...
  await refresh();if(generation===openGeneration&&dialog.open)message('');
...
function upload(file){return new Promise((resolve,reject)=>{
 ...
 xhr.onload=()=>{if(xhr.status===401)document.dispatchEvent(new Event('fermde:unauthorized'));let body;try{body=JSON.parse(xhr.responseText)}catch{reject(Error('Сервер недоступен'));return}xhr.status<300?resolve(body):reject(Error(typeof body.detail==='string'?body.detail:'Ошибка загрузки'))};
...
$('#file-pick').onchange=async e=>{
 if(loading){e.target.value='';return}loading=true;const files=Array.from(e.target.files);e.target.disabled=true;
 ...
 catch(e){message(e.message,true)}finally{e.target.value='';e.target.disabled=false;loading=false}
};
